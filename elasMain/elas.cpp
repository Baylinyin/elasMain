...
#include "triangle.h"
#include "matrix.h"
#include <algorithm> //#include <math.h> max与min所在头文件改变
...
inline int16_t Elas::computeMatchingDisparity (const int32_t &u,const int32_t &v,uint8_t* I1_desc,uint8_t* I2_desc,const bool &right_image) {
  ...
  if (u>=window_size+u_step && u<=width-window_size-1-u_step && v>=window_size+v_step && v<=height-window_size-1-v_step) {
    ...
    for (int16_t d=disp_min_valid; d<=disp_max_valid; d++) {
      ...
      // compute match energy at this disparity       // 这里用到的 Descriptor  数据 为 8 bit类型 共16个，即8*16=128bit
	  
																		// 指令
      xmm6 = _mm_load_si128((__m128i*)(I2_block_addr+desc_offset_1));	//MOVDQA  __m128i  生成 16bit类型 
																		//        _mm_load_si128 为128bit类型，即  xmm6=128/16=8 个字
																				
      xmm6 = _mm_sad_epu8(xmm1,xmm6);								    //PSADBW  操作 16bit 8个字 数据时，对低8位  求一次 1范数
																		//     再对高8位  求一次 1范数；分别储存的下标是 [0],[4]
																				
      xmm5 = _mm_load_si128((__m128i*)(I2_block_addr+desc_offset_2)); 
      xmm6 = _mm_add_epi16(_mm_sad_epu8(xmm2,xmm5),xmm6);               //PADDW
      ...
      sum  = _mm_extract_epi16(xmm6,0)+_mm_extract_epi16(xmm6,4);        //PEXTRW
    ...
    for (int32_t v_can=1; v_can<D_can_height; v_can++) {
      ...
      *(D_can+getAddressOffsetImage(u_can,v_can,D_can_width)) = -1;
      ...
      if (d>=0) {
        ...
        if (d2>=0 && abs(d-d2)<=param.lr_threshold)   //  lr_threshold = 2
          {*(D_can+getAddressOffsetImage(u_can,v_can,D_can_width)) = d; check_disparity++;}
      }
    }
  }
  std::cout << "check_disparity:" << check_disparity << std::endl;
...
void Elas::computeDisparityPlanes (vector<support_pt> p_support,vector<triangle> &tri,int32_t right_image) {
  ...
  std::cout << "valid: " << count_valid << " points" << std::endl;
}
...
void Elas::computeDisparity(vector<support_pt> p_support,vector<triangle> tri,int32_t* disparity_grid,int32_t *grid_dims,
  ...
  std::cout << "matched: " << count_matched << " points" << std::endl;
  delete[] P;
}
